...
import { type Photo } from '../utils/get-photos';
import { useWindowWidth } from '../utils/hooks';
import styles from '../pages/2025/styles.module.css';

interface Events2025Props {
  photos: Photo[];
}

const Events2025: React.FC<Events2025Props> = ({ photos }) => {
  ...
  const displayPhotos = photos.filter((photo) => !photo.original.includes('poster.jpg'));

  const albumPhotos = displayPhotos
    .map((photo) => ({
      src: photo.preview || '',
      width: 256,
      height: (256 / 4) * 3,
  ...
  const fetchPhotos = async (index: number) => {
    const sliced = albumPhotos.slice(index * photosPerRow, (index + 1) * photosPerRow);
    if (sliced.length === 0) return null;
    return sliced;
  };

  const initialPhotos = albumPhotos.slice(0, photosPerRow);

  const onPhotoClick = ({ index }: { index: number }) => {
    window.open(displayPhotos[index].original, '_blank');
  };
  ...
  return (
    <div className={styles.page}>
      ...
      <main className={styles.main}>
        <p className={styles.zing}>Some of the games ever made by really really indie developers</p>
      ...
      <div className={styles.submain}>
        ...
        >
          <MasonryPhotoAlbum photos={[]} columns={photosPerRow} padding={8} />
        </InfiniteScroll>
      </div>
